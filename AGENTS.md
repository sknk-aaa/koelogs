...
本アプリは「AIによる練習メニュー提案」を中核にしたボイストレーニング支援Webアプリである。

目的：
・個人の練習ログ・測定結果・目標をもとに、AIが次に取り組む練習メニューを提案する
・コミュニティ投稿データを集合知として活用し、AI提案の精度と納得感を高める
・日々の練習ログを記録し、練習内容を振り返りやすくする
・録音測定と分析確認は主要導線として `/log` に統合して扱えるようにする
・月単位の振り返り（月メモ + 月サマリー）を蓄積する
・コミュニティ投稿を通じて、練習知見が次のユーザーの提案に還元される循環を作る
...
- /            → /log にリダイレクト（固定）
- /log         : ログトップ（日/月切替、サマリー + アクション + 測定/分析の主要導線）
- /log/new     : 今日のトレーニング入力（RequireAuth）
- /training    : 旧導線 / 補助導線として残るトレーニング音源再生 + 録音測定
- /insights    : 旧導線 / 補助導線として残る分析トップ

## 3.2 分析配下（許可：/insights/*）
分析ページは旧導線 / 補助導線として維持してよい。
新しい主要導線を追加する場合は、まず `/log` への統合を優先して検討すること。
`/insights/*` に新規詳細ページを増やす場合は、役割を分析詳細に限定すること。
現行:
- /insights/time
- /insights/menus
- /insights/notes
...
- 主要UI導線としては、測定・分析のサマリーや関連アクションを `/log` へ寄せる方針とする。
- `/training` `/insights` は現行ルートとして残っていても、役割は `/log` を補助する個別ページとして扱う。
- `/training` `/insights` は内部的にルートが残っていても、通常のユーザー導線からは見えない前提で扱うこと。
- したがって、ユーザー体験・主要機能説明の観点では、測定・分析は実質 `/log` 配下の体験として整理すること。
...
中心は「AIおすすめ生成」と、その根拠となる個人ログ・測定記録・コミュニティ集合知の蓄積である。

1日1ログ、月振り返り、録音測定、コミュニティ投稿は、それぞれ独立した機能であると同時に、AIによる練習メニュー提案の質を高めるための基盤機能として位置づける。
